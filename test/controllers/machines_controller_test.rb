...
class MachinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create machine" do
    assert_difference('Machine.count') do
      post machines_url, params: { machine: { ip: @machine.ip, name: @machine.name, port: @machine.port } }
    end
  ...
  test "should update machine" do
    patch machine_url(@machine), params: { machine: { ip: @machine.ip, name: @machine.name, port: @machine.port } }
